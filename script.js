...
function collectResource(index) {
    ...
    if (HARD_RES.includes(item.name)) {
        ...
        if (!hasPickaxe) {
            log(`需要一把 [镐子] 才能采集 ${item.name}。`, "red");
            return;
        }
    }
...
function startCombat(mob, index) {
    ...
    let imgUrl = ITEM_ICONS[mob.name] || "";
    if (!imgUrl) {
        let baseName = mob.name.replace("狂暴的", "").replace("地狱的", "");
        imgUrl = ITEM_ICONS[baseName] || "";
    }
...
function updateCombatUI() {
    ...
    if (player.hp <= 0) {
        setTimeout(() => { alert("你死了！"); location.reload(); }, 500);
        return;
    }
    ...
    if (healContainer) {
        ...
        for (let [name, count] of Object.entries(player.inventory)) {
            ...
            if (recipe && recipe.type === 'use' && (recipe.effect === 'heal' || recipe.effect === 'food')) {
                ...
                let iconStr = ITEM_ICONS[name] ? `<img src="${ITEM_ICONS[name]}">` : "";
                btn.innerHTML = `${iconStr} ${name} x${count}`;
...
function combatUseItem(name) {
    ...
    let healAmount = recipe.effect === 'food' ? Math.floor(recipe.val / 2) : recipe.val;
    player.hp = Math.min(player.maxHp, player.hp + healAmount);
    if(recipe.effect === 'food') player.hunger = Math.min(player.maxHunger, player.hunger + recipe.val);
    
    combatLog(`使用了 ${name}`, "blue");
    player.inventory[name]--;
    if (player.inventory[name] <= 0) delete player.inventory[name];

    const eDmg = Math.max(1, currentEnemy.atk - Math.floor(Math.random()));
    player.hp -= eDmg;
    combatLog(`敌人造成 ${eDmg} 伤害`, "red");
...
function getItemType(name) {
    ...
    if (name.includes("肉") || name.includes("排") || name.includes("鱼") || name.includes("苹果")) return 'food';
    return 'material';
}
...
function updateInventoryUI() {
    ...
    if (Object.keys(player.inventory).length === 0) {
        list.innerHTML = '<div style="padding:20px;text-align:center;color:#ccc;">背包空空如也</div>';
        return;
    }
    ...
    for (let [name, count] of Object.entries(player.inventory)) {
        if (count > 0) {
            ...
            if (show) {
                ...
                let iconHtml = ITEM_ICONS[name] ? `<img src="${ITEM_ICONS[name]}" class="item-icon">` : "";
                let r = RECIPES.find(x => x.name === name);
                let btnText = (r && r.type === 'build') ? "放置" : (r && r.type === 'equip') ? "装备" : "使用";

                row.innerHTML = `
                    <div style="display:flex; align-items:center; gap:10px; flex:1;">
                        ${iconHtml} <span style="font-weight:bold;">${name}</span>
                    </div>
                    <div style="display:flex; align-items:center; gap:10px;">
                        <b style="color:#999;">x${count}</b>
...
function useItem(name) {
    ...
    if (name === "金苹果") {
        ...
        if (recipe.effect === 'food') {
            ...
            log(`装备了 ${name}！ATK=${player.atk}`);
        }
        else if (recipe.effect === 'hp_max') {
            player.maxHp = recipe.val;
            player.hp = player.maxHp; 
            log(`装备了 ${name}！HP=${player.maxHp}`);
        }
...
function updateCraftUI() {
    ...
    RECIPES.forEach(recipe => {
        ...
        if (show) {
            const row = document.createElement('div');
            row.className = 'list-item';
            let iconHtml = ITEM_ICONS[recipe.name] ? `<img src="${ITEM_ICONS[recipe.name]}" class="item-icon">` : "";
            ...
            let stationHtml = stationMissing ? `<div style="font-size:10px;color:red;">⚠️ ${missingMsg}</div>` : "";
...
function refreshLocation() {
    ...
    offsets.forEach(o => {
        let nx=player.x+o.dx, ny=player.y+o.dy;
        if(nx>=0 && nx<MAP_SIZE && ny>=0 && ny<MAP_SIZE) currentMap[`${nx},${ny}`] = true;
    });
...
function placeBuilding(name) {
    ...
    let newBuild = { name: name, content: name==="工作台"?{}:null };
